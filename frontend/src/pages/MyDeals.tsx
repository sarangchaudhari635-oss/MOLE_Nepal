...
import {
    ...
    Handshake, MessageSquare, Award, FileDown
} from 'lucide-react';
...
const MyDeals = () => {
    ...
    const handleDownloadCertificate = (deal: Transaction) => {
        // Implement PDF certificate generation using browser print
        ...
        const html = `
            ...
            <html>
            <head>
                <title>Circularity Certificate - ${deal.id}</title>
                <style>
                    body { font-family: 'Arial', sans-serif; padding: 40px; color: #1f2937; }
                    .header { text-align: center; border-bottom: 2px solid #0FAB76; padding-bottom: 20px; margin-bottom: 30px; }
                    .logo { font-size: 24px; font-weight: bold; color: #0FAB76; letter-spacing: 2px; }
                    .title { font-size: 32px; font-weight: bold; margin-top: 10px; }
                    .content { font-size: 16px; line-height: 1.6; }
                    .details-table { width: 100%; border-collapse: collapse; margin-top: 30px; }
                    .details-table th, .details-table td { padding: 15px; border-bottom: 1px solid #e5e7eb; text-align: left; }
                    ...
                    .badge { display: inline-block; background-color: #ecfdf5; border: 1px solid #10b981; color: #047857; padding: 8px 16px; border-radius: 20px; font-weight: bold; margin-top: 20px; }
                </style>
            </head>
            <body>
                <div class="header">
                    <div class="logo">MOLE</div>
                    <div class="title">Official Circularity Certificate</div>
                    <div class="badge">98% Synergy Match Completed</div>
                </div>
                <div class="content">
                    <p>This certifies the successful transfer of industrial byproducts between verified partners on the MOLE Circular Economy Platform.</p>
                    
                    <table class="details-table">
                        <tr><th>Transaction ID</th><td>${deal.id}</td></tr>
                        <tr><th>Date Completed</th><td>${new Date().toLocaleDateString()}</td></tr>
                        <tr><th>Material Transferred</th><td><strong>${deal.material}</strong></td></tr>
                        <tr><th>Quantity Transferred</th><td>${deal.amount} units</td></tr>
                        <tr><th>Your Role</th><td>${isSeller ? 'Supplier' : 'Receiver'}</td></tr>
                        <tr><th>Counterparty</th><td>${partnerName || 'Verified Partner'}</td></tr>
                    </table>
                </div>
                <div class="footer">
                    <p>Generated by MOLE Platform • Accelerating the path to Net-Zero</p>
                    <p>This document uses secure blockchain-backed verification. Hash: ${btoa(deal.id).substring(0, 16)}...</p>
                </div>
                <script>
                                    ...
                                    {stageDeals.map(deal => {
                                        ...
                                        return (
                                            <div key={deal.id} className="bg-white rounded-2xl border border-surface-200/60 p-4 shadow-sm hover:shadow-md transition-all group">
                                                ...
                                                {!nextStage && (
                                                    <div className="flex gap-2">
                                                        <div className="flex-1 py-2.5 bg-emerald-50 text-emerald-700 text-[11px] font-bold rounded-xl flex items-center justify-center gap-1.5 border border-emerald-200">
                                                            <Award size={13} /> Completed
                                                        </div>
                                                        <button 
                                                            onClick={() => handleDownloadCertificate(deal)}
                                                            className="px-3 py-2.5 bg-surface-900 hover:bg-black text-white text-[11px] font-bold rounded-xl flex items-center justify-center transition-all shadow-sm"
                                ...
                                <div className="flex items-center justify-between pt-4 border-t border-surface-100">
                                    ...
                                    <div className="flex gap-2">
                                        {nextStage ? (
                                            ...
                                            <button
                                                onClick={() => handleDownloadCertificate(deal)}
                                                className="px-4 py-2.5 bg-surface-900 hover:bg-black text-white text-[12px] font-bold rounded-xl flex items-center gap-2 transition-all shadow-md"
                                            >
